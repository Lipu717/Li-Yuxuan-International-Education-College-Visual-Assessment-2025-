...
//            Vec3b pixel = tempImage.at<Vec3b>(y, x); // 获取像素的BGR值（OpenCV默认BGR顺序）
...
//            imwrite("cropped_cat.jpg", croppedImage); // 保存裁剪后的图像到本地
//
//            // 计算并输出矩形中心坐标
//            Point center((x1 + x2) / 2, (y1 + y2) / 2);
//            cout << "矩形中心像素坐标: (" << center.x << ", " << center.y << ")" << endl;
//
//            // 在原图上绘制最终的红色矩形（可选，标记框选结果）
...
//int main() {
//    // 1. 读取图像
//    image = imread("D:/images/cat.png"); // 替换为你的猫咪图片路径
//    if (image.empty()) {
